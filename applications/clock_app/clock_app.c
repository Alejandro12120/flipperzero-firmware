...
static void clock_render_callback(Canvas* const canvas, void* ctx) {
    ...
    if(timer_start_timestamp != 0 && !state->w_test) {
        ...
        if(state->w_test) canvas_set_font(canvas, FontSecondary);
        ...
        if(!state->w_test) elements_button_left(canvas, state->militaryTime ? "12h" : "24h");
    }
    if(!state->w_test) {
        if(timer_running) {
                        ...
                        if(plugin_state->timerSecs == plugin_state->alert_time) {
                            ...
                            if(plugin_state->lastexp_timestamp + 10 <= curr_ts &&
                               plugin_state->w_test) {
                                ...
                                //TODO FIXME DOLPHIN_DEED(getRandomDeed());
                            }
